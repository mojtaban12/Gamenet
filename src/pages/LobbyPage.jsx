...
import { Check, Copy, Crown, MessageCircle, Smile, UserX, Volume2, VolumeX } from 'lucide-react'
...
export default function LobbyPage() {
    ...
    useEffect(() => {
        function onKicked() {
            toast('از لابی اخراج شدید', 'error', 5000)
            navigate('/rooms')
        }
    ...
    useEffect(() => {
        function onIncoming(e) {
            ...
            setChatFriend(prev => {
                ...
                if (prev) {
                    toast(`پیام جدید از ${senderName} — برای باز کردن کلیک کنید`, 'info', 6000, () => {
    ...
    return (
        <AppShell>
            ...
            <div dir="ltr" className="flex flex-1 min-h-0 min-w-0 gap-3 items-stretch overflow-hidden">
                <div dir="rtl" className="w-60 max-w-[26vw] shrink-0 min-h-0 og-panel flex flex-col overflow-hidden">
                    <div className="og-lobby-col-head px-3">
                        <p className="og-label text-og-muted truncate">افراد لابی</p>
                    </div>
                    ...
                    <section className="flex-1 min-h-0 flex flex-col overflow-hidden border-b border-og/50">
                        <div className="flex items-center justify-between gap-2 px-3 py-2 flex-shrink-0">
                            <p className="og-label text-og-muted truncate">
                                اعضا
                                <span className="text-og-body font-normal mr-1">· {members.length}</span>
                            </p>
                        ...
                        <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-3 pb-2">
                            {members.length === 0 ? (
                                <p className="text-center py-6 text-og-muted text-xs">هنوز کسی نیامده</p>
                            ) : (
                                members.map((m, i) => (
                                    <MemberRow
                    ...
                    <div className="px-3 py-2.5 border-t border-og flex-shrink-0">
                        <button
                            ...
                            {leaving ? 'در حال خروج...' : 'خروج از لابی'}
                        </button>
                    </div>
                </div>

                <div dir="rtl" className="flex-1 min-w-0 min-h-0 flex flex-col gap-3 overflow-hidden">
                            ...
                            <div className="flex items-start justify-between gap-2">
                                <div className="min-w-0 flex-1">
                                    ...
                                    <div className="flex items-center gap-1.5 mt-0.5">
                                        ...
                                        <span className="text-og-muted text-[11px]">{connected ? 'آنلاین' : 'قطع'}</span>
                                    </div>
                                </div>
                                <button
                                    type="button"
                    ...
                    {chatEnabled && <div className="og-panel flex-1 min-h-0 flex flex-col overflow-hidden">
                        <div className="og-lobby-col-head px-5 gap-3">
                            <div className="flex items-center gap-3 min-w-0">
                                <Icon icon={MessageCircle} size="md" className="text-og-accent shrink-0" />
                                <span className="og-title text-sm text-og-body shrink-0">چت لابی</span>
                            </div>
                        </div>
...
function MemberRow({ member, isMe, isCurrentUserHost, speaking, muted, gameInfo, onToggleMute, onKick, onTransferHost }) {
    const m = member
    ...
    return (
        <div
            ...
            <div className={`relative w-7 h-7 og-avatar-ring text-[11px] font-bold shrink-0 transition-all overflow-hidden ${
                ...
                {m.isHost && (
                    <span className="absolute -top-1.5 -right-1.5 text-[9px]" title="میزبان">
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                    ...
                    {isMe && <span className="text-og-accent text-[11px] shrink-0">شما</span>}
                </div>
                <div className="font-mono text-og-muted text-[10px] mt-0.5 flex items-center gap-1">
                    ...
                    {displayIp
                        ...
                        : <span>{m.connected ? 'آنلاین' : 'قطع'}</span>
                    }
                </div>
                {memberChannel && (
            ...
            <div className={`flex items-center gap-1 shrink-0 transition-opacity ${showActions || isCurrentUserHost ? 'opacity-100' : 'opacity-0'}`}>
                ...
                {!isMe && m.userId && (
                    <button
                        ...
                        title={muted ? 'صدا وصل' : 'صدا قطع (فقط برای شما)'}
...
function TypingIndicator({ users }) {
    const label = users.length === 1
        ? `${users[0].username} در حال نوشتن`
        : `${users.length} نفر در حال نوشتن`
    return (
...
function MessageBubble({ msg, currentUserId }) {
    ...
    return (
        <div className={`flex gap-2 animate-fade-in ${isMine ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={`max-w-md flex flex-col gap-1 ${isMine ? 'items-end' : 'items-start'}`}>
                {!isMine && (
                    <span className="text-og-muted text-xs px-1">{msg.senderName}</span>
                )}
                <div className={`px-3.5 py-2 rounded-xl text-sm selectable whitespace-pre-wrap break-words ${
